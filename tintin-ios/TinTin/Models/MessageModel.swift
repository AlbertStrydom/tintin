...
struct MessageModel: Identifiable, Codable, Equatable {
    ...
    static func parseStructuredPayload(_ payload: [String: Any], senderId: String) -> (text: String, type: StructuredType?, groupName: String?, channelName: String?, pollQuestion: String?, stickerEmoji: String?) {
        guard let tt = payload["__tintin_type"] as? String else {
            return (payload["text"] as? String ?? String(data: try? JSONSerialization.data(withJSONObject: payload), encoding: .utf8) ?? "", nil, nil, nil, nil, nil)
        }
        ...
        case "group":
            ...
            return ("[\(gn)] \(t)", .group, gn, nil, nil, nil)
        case "channel":
            let cn = payload["channel_name"] as? String ?? "Channel"
            let t = payload["text"] as? String ?? ""
            return ("📢 [\(cn)] \(t)", .channel, nil, cn, nil, nil)
        case "poll":
            let q = payload["question"] as? String ?? "Poll"
            return ("📊 Poll: \(q)", .poll, nil, nil, q, nil)
        case "sticker":
            let e = payload["emoji"] as? String ?? "🖼️"
            return ("\(e) Sticker", .sticker, nil, nil, nil, e)
        case "call_offer", "call_accept", "call_end":
            let t = payload["reason"] as? String ?? "📞 Call signal"
            return ("📞 \(t)", tt == "call_offer" ? .callOffer : tt == "call_accept" ? .callAccept : .callEnd, nil, nil, nil, nil)
        case "file":
            let fn = payload["file_name"] as? String ?? "File"
            return ("📁 File: \(fn)", .file, nil, nil, nil, nil)
        default:
            return (payload["text"] as? String ?? "", nil, nil, nil, nil, nil)
        }
    }
}
